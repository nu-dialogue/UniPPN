...
current_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(
    os.path.abspath(__file__)
)))))
sys.path.append(current_dir)

from system.policy.lava.utils.nlp import insertSpace, timepat, pricepat, replacements
...
def prepareSlotValuesIndependent(db_path):
    ...
    fin = open(os.path.join(db_path, 'train' + '_db.json'))
    db_json = json.load(fin)
    fin.close()
...
if __name__ == '__main__':
    dic = prepareSlotValuesIndependent("../data/norm-multi-woz/db")
    pickle.dump(dic, open('svdic.pkl', 'wb+'))
