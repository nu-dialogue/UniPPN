...
class MyPPOPolicy(PolicyBase):
    module_name = "ppo_policy"

    def __init__(self, device: torch.device) -> None:
        self.policy_core = PPO(is_train=False, dataset='Multiwoz')

        ppo_model_path = os.path.join(
            ROOT_DPATH, "ConvLab-2/convlab2/policy/ppo/save/best_ppo.pol.mdl"
        )
        self.policy_core.policy.load_state_dict(torch.load(ppo_model_path, map_location=device))
